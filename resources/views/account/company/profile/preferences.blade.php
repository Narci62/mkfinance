<x-app-layout>
    <div class="content content-full">
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            <div>
                <h1 class="flex-grow-1 fs-3 fw-semibold mb-2">{{ __('Préférences') }}</h1>
                <p class="fw-medium my-0 text-muted">
                    {{ __('Gérez les préférences de votre compte.') }}
                </p>
            </div>
        </div>
    </div>
                        ...
                        <div class="col-lg-12">
                            <div class="block block-rounded">
                                <div class="block-header d-block pb-0 mb-0">
                                    <h3 class="block-title">Langue</h3>
                                    <p class="mt-1 fs-sm text-gray-300">
                                        Choisissez la langue d'affichage par defaut
                                    </p>
                                </div>
                                <div class="block-content pt-0 mt-0">
                                    <div class="col-6">
                                        <form action="{{ route('localization') }}" method="GET">
                                            <div class="form-floating mb-4">
                                                <select class="form-select" id="localization" name="locale" aria-label="Langue d'affichage" required>
                                                    <option>Choisissez une option</option>
                                                    @foreach(config('localization.locales') as $locale => $lang)
                                                        <option @if(session()->get('localization') == $locale) selected @endif value="{{ $locale }}">{{__($lang)}}</option>
                                                    @endforeach
                                                </select>
                                                <label class="form-label" for="localization">Langue d'affichage</label>
