        ...
        <div class="row">
                ...
                <div class="block block-rounded block-mode-loading-refresh">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">
                            {{__('Liste complète des Investisseur(s)')}}
                        </h3>
                        ...
                        <table class="table table-striped table-hover table-borderless table-vcenter fs-sm">
                            ...
                            <tbody>
                                ...
                                <tr class="text-center">
                                    <td colspan="3" >{{__('Aucune donnée')}}</td>
                                </tr>
                                @else
                                @foreach($investors as $row)
                                <tr>
                                    <td>
                                        <span class="fw-semibold">{{ $row->investors->matricule }}</span>
                                    </td>
                                    <td class="d-none d-xl-table-cell">
                                        <span class="fs-sm text-muted">{{ $row->investors->firstname . " " . $row->investors->lastname }}</span>
                                    </td>
                                    <td>
                                        <span class="fw-semibold">{{ \Carbon\Carbon::parse($row->created_at)->translatedFormat('l j Y') }}</span>
                                    </td>
            ...
            <div class="block block-rounded text-center d-flex flex-column h-100 mb-0">
                    <div class="block-content block-content-full flex-grow-1">
                        ...
                        <div class="fs-1 fw-bold">{{$project->investment->count()}}</div>
                        <div class="text-muted mb-3"> {{ __('Inestisseur(s) trouvé(s)') }} </div>
